...
@section('content')
    ...
    <div class="container mx-auto flex items-center">
        <!-- First Tab for "Alle" -->
        <button id="tab-" onclick="filterReservations('')" class="tab-button rounded-t p-4 text-sm mr-2 flex items-center bg-gray-600 text-white flex items-center">
        ...
        @foreach($rooms->sortBy('name') as $room)
            <button id="tab-{{ $room->id }}" onclick="filterReservations('{{ $room->id }}')" class="tab-button rounded-t p-4 text-sm mr-2 flex items-center hover:text-black text-gray-700 bg-gray-200">
                <svg width="24" height="24" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="mr-2">
        ...
        <table class="w-full bg-gray-700 text-white rounded-lg table-fixed text-left">
            <thead>
                <tr>
                    ...
                    <th class="border-b-2 px-4 py-2 border-gray-500 font-medium text-sm">Startdatum</th>
                    <th class="border-b-2 px-4 py-2 border-gray-500 font-medium text-sm">Enddatum</th>
            ...
            <tbody id="reservationTableBody">
                @foreach($reservations as $reservation)
                    <tr class="reservation-row text-gray-300" data-room-id="{{ $reservation->room_id }}">
                        ...
                        <td class="border-b px-4 py-2 border-gray-600 text-sm">{{ $reservation->user->name }}</td>
                        <td class="border-b px-4 py-2 border-gray-600 text-sm">{{ \Carbon\Carbon::parse($reservation->start_date)->format('d.m.Y') }}</td>
                        <td class="border-b px-4 py-2 border-gray-600 text-sm">{{ \Carbon\Carbon::parse($reservation->end_date)->format('d.m.Y') }}</td>
                        <td class="border-b px-4 py-2 border-gray-600 text-right">
                            <form action="{{ route('reservations.cancel', $reservation) }}" method="POST" onsubmit="return confirm('Möchten Sie diese Reservierung wirklich störnieren?');">
                                @csrf
                                @method('DELETE')
    ...
    <script>
        function filterReservations(roomId) {
